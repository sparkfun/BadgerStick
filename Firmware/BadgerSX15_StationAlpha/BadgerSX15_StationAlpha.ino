...
void loop() {
  ...
  if ( bytes_received > 0 ) {
    ...
    if ( badge_id == 0 ) {
     ...
     Serial.println( "...touched for the very first time..." );
     Serial.println( "Let's get you a number, son." );
     assign_id();
      
    } else {
     
     Serial.print( "It's our old friend #" );
...
void assign_id () {
  ...
  word last_id = word ( EEPROM.read( ID_ADDRESS_H ), EEPROM.read( ID_ADDRESS_L ) );
  
  Serial.print( "Last ID assigned was " );
  Serial.println( last_id, HEX );
  
  word new_id = last_id + 1;
  
  Serial.print( "Assigning ID #" );
  Serial.println( new_id );
  
  byte new_id_l = new_id & 0x00FF;
  byte new_id_h = ( new_id >> 8 ) & 0x00FF;
  
  byte msg[] = { 0x1D, new_id_h, new_id_l }; 
  softy.begin(600);
  transmitMessage( msg, 3 );
  
//  EEPROM.write( ID_ADDRESS_L , new_id_l );
//  EEPROM.write( ID_ADDRESS_H , new_id_h );
  
//  Serial.println( "EEPROM Update Success" );
